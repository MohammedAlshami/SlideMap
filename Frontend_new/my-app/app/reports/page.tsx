...
export const Blog = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/reports?page=1")
      .then((response) => response.json())
      .then((data) => {
        // Set the fetched news list
        setNewsList(data.data);
        setFilteredNews(data.data); // Initialize filtered news with all news articles
  ...
  const handleSearchClick = (e) => {
    ...
    window.location.href = `http://localhost:3000/reports?search=${encodeURIComponent(searchQuery)}`;
  };
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <>
              ...
              {filteredNews.map((news) => (
                <a
                  key={news.index}
                  href={`/reports/report?index=${news.id}`}
